...
try {
		...
		} elseif (isset($_GET["getItems"])) {
			print_r(json_encode( $client->products->get( null, array( 'filter[limit]' => 5000, 'filter[meta]'=>'true'))));
				//print_r( $client->products->get( null, array( 'category' => $_GET["category"] ) ) )
		}elseif (isset($_GET["productId"])) {
			$product_id = $_GET['productId'];
			print_r(json_encode( $client->products->get( $product_id ) ));
				
		}elseif (isset($_GET["getCatSlug"])) {
			$category_slug = $_GET['getCatSlug'];
			//print_r(json_encode( $client->products->get_categories( $category_id )) );
			print_r(json_encode( $client->products->get( null, array( 'filter[limit]' => 25, 'filter[category]'=>$category_slug))));
		}elseif (isset($_GET["getIndex"])) {
			print_r(json_encode( $client->index->get() ));
		} elseif (isset($_GET["getCountries"])) {
			require_once("../wp-config.php");
			$data = WC()->countries->get_countries();
			print_r(json_encode($data));
		} elseif (isset($_GET["getOrder"])) {
			print_r(json_encode( $client->orders->get( $_GET["getOrder"]  ) ));
		}
